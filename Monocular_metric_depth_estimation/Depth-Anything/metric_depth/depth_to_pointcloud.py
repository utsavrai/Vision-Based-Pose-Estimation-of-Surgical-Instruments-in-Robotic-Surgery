...
FINAL_HEIGHT = 540
FINAL_WIDTH = 960
INPUT_DIR = '/home/utsav/IProject/FoundationPose/demo_data/LND/rgb'
OUTPUT_DIR = '/home/utsav/IProject/FoundationPose/demo_data/LND/depth_DA'
...
def process_images(model):
    ...
    for image_path in tqdm(image_paths, desc="Processing Images"):
        try:
            ...
            rpred = cv2.resize(pred, (FINAL_WIDTH, FINAL_HEIGHT), interpolation=cv2.INTER_NEAREST)
            scale = 1000
            # Save depth map as a 16-bit image
            depth_map_image_path = os.path.join(OUTPUT_DIR, os.path.splitext(os.path.basename(image_path))[0] + "_depth.png")
            cv2.imwrite(depth_map_image_path, (rpred * scale).astype(np.uint16))  # scale factor for depth values
