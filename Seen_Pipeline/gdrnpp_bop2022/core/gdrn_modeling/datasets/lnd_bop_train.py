...
class LND_BOP_TRAIN_Dataset(object):
    """lnd bop train splits."""
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get("dataset_root", osp.join(DATASETS_ROOT, "BOP_DATASETS/lnd/train"))
        ...
        self.models_root = data_cfg["models_root"]  # BOP_DATASETS/lnd/models
        self.scale_to_meter = data_cfg["scale_to_meter"]  # 0.001

        self.with_masks = data_cfg["with_masks"]
        self.with_depth = data_cfg["with_depth"]

        # self.height = data_cfg["height"]  # 540
        # self.width = data_cfg["width"]  # 960
        self.height = 540
        self.width = 960
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.lnd_full.id2obj.items() if obj_name in self.objs]
        ...
        for scene_id, int_im_id in tqdm(scene_im_ids):
            ...
            for anno_i, anno in enumerate(gt_dict[str_im_id]):
                ...
                inst = {
                    ...
                    "xyz_path": None,  #  no need for train
                }
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.lnd_full.obj2id[obj_name]:06d}.ply",
                ),
...
def get_lnd_metadata(obj_names, ref_key):
...
SPLITS_LND = dict(
    lnd_bop_train=dict(
        name="lnd_bop_train",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lnd/train"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lnd/models"),
        objs=ref.lnd_full.objects,
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/lnd/train_targets_bop19.json"),
...
# single obj splits for lnd bop train
for obj in ref.lnd_full.objects:
    for split in [
        "bop_train",
    ]:
        name = "lnd_{}_{}".format(obj, split)
        ann_files = [
            osp.join(
                DATASETS_ROOT,
                "BOP_DATASETS/lnd/image_set/{}_{}.txt".format(obj, split),
            )
        ]
        if name not in SPLITS_LND:
            SPLITS_LND[name] = dict(
                name=name,
                dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lnd/"),
                models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lnd/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_LND:
        used_cfg = SPLITS_LND[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, LND_BOP_TRAIN_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="lnd",  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_lnd_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_LND.keys())
...
def train_vis():
    ...
    for d in dicts:
        ...
        for _i in range(len(annos)):
            ...
            if "train" not in dset_name.lower():
