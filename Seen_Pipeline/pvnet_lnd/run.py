...
def run_demo():
    ...
    output_directory = 'demo_output_directory'
    torch.manual_seed(0)
    ...
    for demo_image_path in demo_images:
        demo_image = np.array(Image.open(demo_image_path)).astype(np.float32)
        inp = (((demo_image / 255.) - mean) / std).transpose(2, 0, 1).astype(np.float32)
        inp = torch.Tensor(inp[None]).cuda()

        # Generate a save path based on the input image name
        base_name = os.path.basename(demo_image_path)
        name, ext = os.path.splitext(base_name)
        save_path = os.path.join(output_directory, f"{name}_output{ext}")

        with torch.no_grad():
            output = network(inp)

        visualizer.visualize_demo(output, inp, meta, save_path=save_path)
