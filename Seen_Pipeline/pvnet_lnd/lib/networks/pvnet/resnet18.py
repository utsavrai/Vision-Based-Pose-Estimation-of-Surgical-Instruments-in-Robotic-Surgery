...
class Resnet18(nn.Module):
    ...
    #     if fm.shape[2] != x4s.shape[2] or fm.shape[3] != x4s.shape[3]:
    #         fm = nn.functional.interpolate(fm, size=(x4s.shape[2], x4s.shape[3]), mode='bilinear', align_corners=False)

    #     fm = self.conv4s(torch.cat([fm, x4s], 1))
    #     fm = self.up4sto2s(fm)

    #     if fm.shape[2] != x2s.shape[2] or fm.shape[3] != x2s.shape[3]:
    #         fm = nn.functional.interpolate(fm, size=(x2s.shape[2], x2s.shape[3]), mode='bilinear', align_corners=False)

    #     fm = self.conv2s(torch.cat([fm, x2s], 1))
    #     fm = self.up2storaw(fm)

    #     if fm.shape[2] != x.shape[2] or fm.shape[3] != x.shape[3]:
    #         fm = nn.functional.interpolate(fm, size=(x.shape[2], x.shape[3]), mode='bilinear', align_corners=False)
