...
p = {
    ...
    "dataset_path": "/home/utsav/IProject/data/dataset/lnd1",
...
class AppWindow:
    ...
    def scene_load(self, scenes_path, scene_num, image_num):
        scene_path = os.path.join(scenes_path, f"{scene_num}")
        rgb_path = os.path.join(scene_path, "rgb", f"{image_num}" + ".png")
        if not os.path.exists(rgb_path):
            print(f"RGB image {rgb_path} does not exist. Skipping image {image_num}.")
            return
        self._annotation_changed = False
        ...
        depth_path = os.path.join(scene_path, "depth", f"{image_num}" + ".png")
