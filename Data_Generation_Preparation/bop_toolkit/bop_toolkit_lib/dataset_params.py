...
def get_split_params(datasets_path, dataset_name, split, split_type=None):
    ...
    # HOPE.
    ...
    elif dataset_name == "lnd1":
        p["scene_ids"] = {"train": [1], "test": [1]}[split]
        p["im_size"] = (960, 540)

        if split == "test":
            p["depth_range"] = (50, 200)
            p["azimuth_range"] = (0, 2 * math.pi)
            p["elev_range"] = (0, 0.5 * math.pi)
    elif dataset_name == "lnd2":
        p["scene_ids"] = {"train": [1], "test": [1]}[split]
        p["im_size"] = (960, 540)
        ...
        raise ValueError("Unknownx BOP dataset ({}).".format(dataset_name))
